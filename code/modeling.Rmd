...
couldabeens_t <- rbind(couldabeens_t, couldabeens)
```

```{r}
write_csv(couldabeens_t, "couldabeens_t.csv")
```
...
```{r}
ggplot(couldabeens, aes(x = Year, y = prop)) +
  geom_point() +
  stat_smooth(method = "lm", se = FALSE)
```

# Hypothesis Test: Splitting the Data

```{r}
lm <- lm(prop ~ postMoneyball, data = couldabeens)
summary(lm)
```
